...
void N_vs_pT_Forward_020(){
    ...
    // Define the canvas
    TCanvas* c1 = new TCanvas("c1","N_vs_pT_Forward_020", 480, 360);
    ...
    TH2D* h = new TH2D("h","Average Number Density vs p_{T}^{lead} for multiplicity class 0 to 20 in Toward Region",50,0,5,50,0,4);
	...
	// Loop to find delta phi
    	for(int j=0; j<ntrks; j++) {
    		...
    		if((delphi[j]<PI && eta[j]>-2.5 && eta[j]<2.5)){	
    			n_forward++;
    		}
